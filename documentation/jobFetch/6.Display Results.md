...
sequenceDiagram
    ...
    activate JobsContext
    OpenPositions->>JobsContext: - `allJobs` (with travel data)<br/>- `searchString`<br/>- `isJobsLoading`<br/>- `jobFetchError`<br/>- `travelFetchError`
    deactivate JobsContext
    ...
    alt isJobsLoading is true
        ...
        alt filteredJobs.length === 0
            ...
            OpenPositions->>OpenPositions: Render job count message:<br/>"Found [X] jobs in total for [searchString]"
            OpenPositions->>OpenPositions: If any filters are active,<br/>also display: "Filtered [filteredJobs.length] jobs"
