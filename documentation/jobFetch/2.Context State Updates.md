...
sequenceDiagram
    ...
    activate SearchInput
    SearchInput->>JobsContext: Call `performJobFetch()` with<br/>route: "/jobs/search"<br/>method: POST<br/>body: { search_string: inputValue }<br/>credentials: "include"
    Note over JobsContext: Fetch starts immediately<br/>(detailed in Phase 4.1)<br/>useFetch hook called with route "/jobs/search"
    deactivate SearchInput
