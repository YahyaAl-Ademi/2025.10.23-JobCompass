...
sequenceDiagram
    ...
    Note over SearchInput: After successful validation<br/>(from Phase 1)
    ...
    activate SearchInput
    SearchInput->>JobsContext: Call `performJobFetch()` with<br/>method: "POST"<br/>body: JSON.stringify({ search_string: inputValue })<br/>credentials: "include"
    Note over JobsContext: This triggers useFetch hook<br/>configured with route "/jobs/search"<br/>callback: handleJobFetchResults()
    deactivate SearchInput
