...
sequenceDiagram
    ...
    SearchInput->>SearchInput: Get input value from ref<br/>`inputRef.current?.value`
    SearchInput->>SearchInput: Clean input text with<br/>`cleanUpText(inputValue)`<br/>%% from: client/src/util/cleanUpText.js
    SearchInput->>SearchInput: Validate cleaned input with<br/>`validateJobInput({ text: cleanedInput })`<br/>%% from: client/src/util/searchValidation.js
    ...
    alt Validation fails
        ...
        AlertMessage->>User: Display validation error<br/>(e.g., "Please add at least one job title")
        deactivate AlertMessage
        Note over SearchInput,User: Search process stops here<br/>function returns early
        deactivate SearchInput
    else Validation succeeds
        activate SearchInput
        Note over SearchInput: Proceed to Phase 2<br/>(See navigation above)<br/>Continue with context updates
        deactivate SearchInput
    end
```
