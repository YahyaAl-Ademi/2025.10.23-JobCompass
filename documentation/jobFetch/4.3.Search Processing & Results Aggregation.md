...
sequenceDiagram
    ...
    alt Valid search_terms
        ...
        loop For each search word (index i)
            JobController->>NeonDB: Check if `jobWord` exists in `search_words`
            activate NeonDB
            alt Word cached in DB
                ...
                alt i >= 2 AND searchWords.length > 2
                    ...
                    JobController->>RealJobSearch: call `rapidAPIfetch(jobWord)`
                else
                    JobController->>RealJobSearch: call `rapidAPIfetch(jobWord)` (immediate)
                end
